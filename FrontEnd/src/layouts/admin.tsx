...
import { 
  Menu, X, Home, Users, BarChart3, Settings, 
  FileText, ShoppingCart, Bell, Search, ChevronDown,
  LogOut, User, Package, TrendingUp, DollarSign, Activity
} from 'lucide-react';
...
const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    { icon: Users, label: 'Users', active: false, badge: '12' },
    { icon: ShoppingCart, label: 'Orders', active: false, badge: '5' },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-50 to-slate-100">
          ...
          <nav className="flex-1 space-y-2">
            {menuItems.map((item, index) => (
              <a
                key={index}
                href="#"
                className={`flex items-center justify-between gap-3 px-4 py-3.5 rounded-xl transition-all duration-200 group ${
                  item.active
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon size={22} className={item.active ? '' : 'group-hover:scale-110 transition-transform'} />
                  <span className="font-medium">{item.label}</span>
                </div>
                {item.badge && (
                  <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                    item.active ? 'bg-white/20 text-white' : 'bg-blue-100 text-blue-600'
                  }`}>
                    {item.badge}
                  </span>
                )}
      ...
      {/* Main Content */}
      <main className="pt-20 lg:ml-72 p-4 lg:p-8">
       <Outlet/>
      </main>
    </div>
  );
